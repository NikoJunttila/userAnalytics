...
func (apiCfg *apiConfig) handlerChangePass(w http.ResponseWriter, r *http.Request, user database.User) {
	...
	if !comparedPass {
		respondWithError(w, 400, "Old password not correct")
		return
	}
	...
	respondWithJson(w, 201, "Updated password")
}
...
func (apiCfg *apiConfig) handlerForgotPass(w http.ResponseWriter, r *http.Request, emailSecret string) {
	...
	respondWithJson(w, 200, "Sent password reset link to your email.")
}
...
func (apiCfg *apiConfig) HandlerInitPassReset(w http.ResponseWriter, r *http.Request) {
	...
	token := params.Token
	if token == "" {
		respondWithError(w, 400, "Error getting reset token:")
		return
	}
	...
	if time.Now().After(resetInfo.Expiration) || !resetInfo.Valid {
		respondWithError(w, 400, "Token expired or invalid try again with new token")
		return
	}
